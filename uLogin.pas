...
begin
  ...
  ShowMessage('Срок действия пароля истекает '+datetostr(DM.adodsSecurityGetRightspasswordexpire.asdatetime)+'. Необходимо обратиться в ИВЦ для смены пароля.');
   if  DM.adodsSecurityGetRightspasswordexpire.asdatetime<=date then
  begin
   ShowMessage('Срок действия пароля закончен. Необходимо обратиться в ИВЦ для смены пароля.');
   Exit;
  end;
...
{  if((Edit1.Text='nonStyle')and(Edit2.Text='dfqn,tq3'))or
    ((Edit1.Text='озо')and(Edit2.Text='Fr56Bn7'))or
    ((Edit1.Text='test')and(Edit2.Text='test'))or
    ((Edit1.Text='BSV')and(Edit2.Text='8arZYB24'))or
    ((Edit1.Text='озо1')and(Edit2.Text='2TnKUiz8'))
}  then
      begin
