object fperstudentgroup: Tfperstudentgroup
  Left = 0
  Top = 0
  BorderStyle = bsSizeToolWin
  Caption = #1055#1077#1088#1077#1074#1086#1076' '#1089#1090#1091#1076#1077#1085#1090#1072' '#1074' '#1076#1088#1091#1075#1091#1102' '#1075#1088#1091#1087#1087#1091
  ClientHeight = 148
  ClientWidth = 238
  Color = clSkyBlue
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object SpeedButton1: TSpeedButton
    Left = 156
    Top = 107
    Width = 71
    Height = 30
    Caption = #1054#1090#1084#1077#1085#1072
    Glyph.Data = {
      
      E9EEE2E8EDFFFFFBA5A6D12520B34543B1F3F5FAF0F4F2E8ECEE}
    OnClick = SpeedButton1Click
  end
  object SpeedButton2: TSpeedButton
    Left = 156
    Top = 71
    Width = 71
    Height = 30
    Caption = #1054#1050
    Glyph.Data = {
      
      F1F1ECF1F1EDF1F1E5EEEAF7F6FBFFFFFFFDFAFFE4ECE9ECF2F1}
    OnClick = SpeedButton2Click
  end
  object Label1: TLabel
    Left = 8
    Top = 5
    Width = 65
    Height = 13
    Caption = #1060#1072#1082#1091#1083#1100#1090#1077#1090
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label2: TLabel
    Left = 8
    Top = 51
    Width = 27
    Height = 13
    Caption = #1050#1091#1088#1089
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label3: TLabel
    Left = 8
    Top = 97
    Width = 41
    Height = 13
    Caption = #1043#1088#1091#1087#1087#1072
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object ComboBox1: TComboBox
    Left = 8
    Top = 70
    Width = 121
    Height = 21
    Enabled = False
    TabOrder = 0
    OnCloseUp = ComboBox1CloseUp
    Items.Strings = (
      '1'
      '2'
      '3'
      '4'
      '5'
      '6')
  end
  object DBLookupComboboxEh1: TDBLookupComboboxEh
    Left = 8
    Top = 24
    Width = 195
    Height = 21
    EditButtons = <>
    KeyField = 'ID'
    ListField = 'FN'
    ListSource = DSFak
    TabOrder = 1
    Visible = True
    OnCloseUp = DBLookupComboboxEh1CloseUp
  end
  object DBLookupComboboxEh2: TDBLookupComboboxEh
    Left = 8
    Top = 116
    Width = 121
    Height = 21
    Enabled = False
    EditButtons = <>
    KeyField = 'id'
    ListField = 'GroupName'
    ListSource = DM.DSGroupa
    TabOrder = 2
    Visible = True
  end
  object DSFak: TDataSource
    DataSet = ADODSFak
    Left = 80
    Top = 184
  end
  object ADODSFak: TADODataSet
    Connection = DM.ADOCDecanat
    CursorType = ctStatic
    CommandText = 
      'SELECT ID, '#13#10'case when Department=1 then '#39'(O) '#39' else '#39'('#1047') '#39' end+' +
      'FacultyShortName as FN'#13#10'FROM Faculty'#13#10'order by Department'
    Parameters = <>
    Left = 160
    Top = 168
    object ADODSFakID: TAutoIncField
      FieldName = 'ID'
      ReadOnly = True
    end
    object ADODSFakFN: TWideStringField
      FieldName = 'FN'
      ReadOnly = True
      Size = 54
    end
  end
end
