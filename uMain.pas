...
begin
  ...
  Perevod.fpikazadd.StringGrid1.Cells[1, 0] := 'Фамилия';
  Perevod.fpikazadd.StringGrid1.Cells[2, 0] := 'Имя';
  Perevod.fpikazadd.StringGrid1.Cells[3, 0] := 'Отчество';
  Perevod.fpikazadd.StringGrid1.Cells[4, 0] := 'Курс';
  Perevod.fpikazadd.StringGrid1.Cells[5, 0] := 'Группа';
  ...
  //Процедура для построения отчета по результатам сдачи предметов
...
begin
  ...
  //Процедура для вывода дисциплин не связаных с рабочим планом
...
DMreports.frxRNoSinhron.ShowReport();
 ...
 //Необходимые комбинации с закладками
...
//Необходимые комбинации с закладками, а также заполнение нужных полей диапозоном дат
...
ShowMessage('Завершено');
 Gauge1.Visible:=false;
  ...
  //Вывод отчета и необходимые комбинации с закладками
...
DMreports.frxRSexVuzKurs.ShowReport;
   ...
   //Заполнения полей необходимой информацией
...
edit8.Text:=dm.adodsStudentDataNameF.AsString+' '+dm.adodsStudentDataNamei.AsString+' '+dm.adodsStudentDataNameo.AsString;
   ...
   //Заполнения полей необходимой информацией, операции с закладками
...
//Необходимые действия с закладками
...
begin
  case DBRadioGroup2.ItemIndex of
   ...
   //Выпуск группы, перенос всех студентов группы в архив
...
begin
  ...
  // удаление студента
  if (TTVmain.Selected.Level = 2) then
    if (MessageDlg('Внимание! Будут выпущены все студенты группы ' + #39 +
          TTVmain.Selected.Text + #39 + '! Продолжить?', mtConfirmation,
    ...
    //Закрытие програмы, проверка на изменение данных
...
Loading.Close;
  ...
  begin
    if (dm.ADODSGroups.RecordStatus * [rsNew, rsModified]) <> [] then
      if (MessageBox(handle, 'Имеются не сохраненные данные.Сохранить?',
  ...
  Application.Terminate;
     ...
     //Создание главной формы,запись в лог данных о входе
...
//Создание и показ формы по экспорту группы в диплом
...
ExportDiplom.FExportDiplom.ShowModal;
 ...
 //Построения дерева факультетов, групп и студентов
...
begin
//Если текущий месяц меньше августа, то присвоить переменной значение текущего года
  if (monthOf(date) < 8) then
    CurrentYear := yearof(date)
    //В противном случае текущий год увеличинный на 1
  else
    CurrentYear := yearof(date) + 1;
   //Строим в дереве архив с пустыми полями
  for i := 0 to 7 do
  begin
  ...
  Kurs6 := TTVmain.Items.AddChild(RootNode, 'курс');
  //Присваиваем каждому курсу индекс и изображение с номером курсв
  Kurs1.ImageIndex := 2;
 ...
 //Открываем DataSet  с группами, по правам доступа
  dm.ADODSGroups.Close;
  ...
  //сбрасываем сортировку и очищаем фильтр в ДАТАСЕТЕ  для отображения студентов
  dm.ADODSStudentList.Close;
  ...
  fLoadTreeProgress.PBKurs.Max := dm.ADODSStudentList.RecordCount;
   //Делаем дерево доступным
  TTVmain.Enabled := true;
 //Циклам добавляем группы в дерево
  while not(dm.ADODSGroups.Eof) do
  begin
    if (dm.ADODSGroupsStatus.AsVariant = 0) then
    begin
    //Добавляем группу в соответствующий курс
      ...
      //Присваиваем изображения группам
    ...
    //проверка на добавление обучающихся студентов
      if ((dm.ADODSStudentListStatus.AsInteger = 0) or
        ...
        //Добавление студентов в архив в зависимости от статуса и года окончания
        else
        begin
          ...
          begin
            ...
            aExpelledNode[dff] := TTVmain.Items.AddChild
              (aArchiveYearNode[dff], 'Отчисленные');
            aGraduatedNode[dff] := TTVmain.Items.AddChild
              (aArchiveYearNode[dff], 'Выпускники');
            aDeletedNode[dff] := TTVmain.Items.AddChild
              (aArchiveYearNode[dff], 'Удаленные');
        ...
        begin
          ...
          begin
           ...
           //Добавление студентов в архив в зависимости от статуса
    ...
    //Присваивание изображений студентам в зависимости от метки
...
//Обновление датасетов необходимых для настройки прав доступа
...
begin
  ...
  if (TTVmain.Selected.Level = 3) then
    if (MessageDlg('Отчислить студента ' + #39 + TTVmain.Selected.Text + #39 +
  ...
  while (Searching) and (Noddy <> nil) do
      ...
      then
        fmain.TTVMain.Selected.MoveTo(noddy,naAddChild)
          ...
          //Перевод студента в академический отпуск
...
begin
      if (TTVmain.Selected.Level = 3) then
    if (MessageDlg('Отправить в академический отпуск студента ' + #39 +
...
begin
  ...
  if (TTVmain.Selected.Level = 3) then
    if (MessageDlg('Восстановить студента ' + #39 + TTVmain.Selected.Text +
...
begin
    ...
    dm.ADODSGroups.Close;
    dm.ADODSGroups.open;
    GroupNode := TTVmain.Items.AddChild(TTVmain.Selected, 'Безымянная');
...
begin
  ...
  if (TTVmain.Selected.Level = 2) then
    if (MessageDlg(
        'Внимание! Будут переведены  на следующий курс студенты группы ' +
          #39 + TTVmain.Selected.Text + #39 + '! Продолжить?', mtConfirmation,
        [mbYes, mbNo], 0) = mrYes) then
    begin
  ...
  //перевод студента в слушатели
procedure TfMain.N22Click(Sender: TObject);
begin
     if (TTVmain.Selected.Level = 3) then
    if (MessageDlg('Перевести в слушатели ' + #39 +
          TTVmain.Selected.Text + #39 + '?', mtConfirmation, [mbYes, mbNo], 0)
        = mrYes) then
    begin
...
//  ds := inttostr(CurrentYear1) + '0101';
//  if (TTVmain.Selected.Level = 3) then
//    if (MessageDlg('Удалить студента ' + #39 + TTVmain.Selected.Text + #39 +
//          '?', mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
//    begin
//      dm.ADOSQLExecutor.CommandText :=
...
begin
 ...
 ShellExecute(Handle,
  'open', PChar('Руководство пользователя.chm'), nil, nil, SW_SHOWNORMAL);
end;
...
begin
  ...
  // удаление студента
  if (TTVmain.Selected.Level = 3) then
    if (MessageDlg('Внимание! Студент  ' + #39 +
          TTVmain.Selected.Text + #39 + ' будет выпущен. Продолжить?', mtConfirmation,
        [mbYes, mbNo], 0) = mrYes) then
    begin
      dm.adodsStudentData.Close;
      dm.ADOSQLExecutor.CommandText :=
...
begin
  // удаление группы
  if (TTVmain.Selected.Level = 2) then
    if (MessageDlg('Внимание! Будут удалены все студенты группы ' + #39 +
          TTVmain.Selected.Text + #39 + '! Продолжить?', mtConfirmation,
        [mbYes, mbNo], 0) = mrYes) then
    begin
      dm.ADOSQLExecutor.CommandText :=
...
Application.CreateForm(TfResizePhoto, fResizePhoto);
  ...
  ShowMessage('Выбирите другой формат изображения');
...
begin
  ...
  DMreports.frxRSlowStudent.Variables['y'] := '''' + s + 'ей' + '''';
...
begin
  ...
  DMreports.frxSessionResultSpecNap.Variables['y'] := '''' + s + 'ей' + '''';
  ...
  DMreports.frxROtlichFac.Variables['y'] := '''' + s + 'ей' + '''';
...
DMreports.ADODSSessionResultComers.Parameters[2].Value :=  DateTimePicker5.Date;
  ...
  DMreports.frxRSessionResultComers.Variables['y'] := '''' + s + 'ей' + '''';
  DMreports.frxRSessionResultComers.Variables['osn'] := ''''  + 'коммерческой' + '''';
 ...
 DMreports.ADODSSesionResult.Parameters[2].Value :=  DateTimePicker5.Date;
  ...
  DMreports.frxRSlowCathedr.Variables['y'] := '''' + s + 'ей' + '''';
...
DMreports.ADODSSessionResultComers.Parameters[2].Value :=  DateTimePicker5.Date;
  ...
  DMreports.frxRSessionResultComers.Variables['x'] := '''' + ComboBox3.Text + '''';
  DMreports.frxRSessionResultComers.Variables['y'] := '''' + s + 'ей' + '''';
  DMreports.frxRSessionResultComers.Variables['osn'] := ''''  + 'бюджетной' + '''';
  DMreports.frxRSessionResultComers.ShowReport();
  end;
    end;
...
begin
  ...
  fDirectory.Caption := 'Учебные планы(Выбор)';
...
s:=s+#13+'Фамилия: '+dm.ADODSOldNamestoryOldName.AsString+' до '+ datetostr(dm.ADODSOldNamestoryDateOfChange.AsDateTime)+' Основание: '+dm.ADODSOldNamestoryReason.AsString ;
if dm.ADODSOldNamestoryNamei.AsBoolean then
s:=s+#13+'Имя: '+dm.ADODSOldNamestoryOldName.AsString+' до '+ datetostr(dm.ADODSOldNamestoryDateOfChange.AsDateTime) +' Основание: '+dm.ADODSOldNamestoryReason.AsString ;

if dm.ADODSOldNamestoryNameo.AsBoolean then
s:=s+#13+'Отчество: '+dm.ADODSOldNamestoryOldName.AsString+' до '+ datetostr(dm.ADODSOldNamestoryDateOfChange.AsDateTime)+' Основание: '+dm.ADODSOldNamestoryReason.AsString  ;
...
if (dm.adodsWorkCurriculumidPlan.Value > 0) then
  begin
    ...
    fDirectory.ShowModal;
    fDirectory.Caption := 'Названия групп по номеру специальности/направления';
  end;
end;
   //заполнение полей, вывод отчета
...
begin
  ...
  DMreports.frxRVUSResultSessionFak.Variables['y'] := '''' + s + 'ей' + '''';
  ...
  DMreports.frxRVUZSessionResultSpec.Variables['y'] := '''' + s + 'ей' + '''';
 ...
 DMreports.ADODSSessionResultSpecNap.Parameters[2].Value := DateTimePicker7.Date;
  ...
  DMreports.  frxVUZKursSpecNapResulrSession.Variables['y'] := '''' + s + 'ей' + '''';
...
if dm.ADODSFacultiesDepartment.AsInteger=1 then
DMreports.frxRInostr.Variables['otd'] := '''' +'Очное отделение'+ ''''
else
DMreports.frxRInostr.Variables['otd'] := '''' +'Заочное отделение'+ '''';
...
begin
  ...
  DMreports.frxRStipCom.Variables['s'] := '''' + s + 'ей' + '''';
...
begin
  ...
  DMreports.frxRUved1.Variables['session'] := '''' + s + 'юю' + '''';
...
begin
   if SpeedButton57.Caption='Добавить новый' then
         begin
   ...
   DBESpecialityCode.ReadOnly:=false;
     SpeedButton57.Caption:='Сохранить'
         end
         else
         begin
   ...
   DBESpecialityCode.ReadOnly:=true;
      dm.adodsWorkCurriculum.Close;
        ...
        dm.adodsWorkCurriculum.open;
        SpeedButton57.Caption:='Добавить новый'
         end;
end;
...
DMreports.frxRSpravkaOtch.Variables['po'] := '''' +edit9.Text+ '''';
 DMreports.frxRSpravkaOtch.Variables['rector'] := '''' +edit10.Text+ '''';
  ...
  DMreports.frxRSpravkaOtch.Variables['dip'] := '''' +'Выдан диплом установленного образца '+edit11.Text+ ''''  ;
 if CheckBox5.Checked then
 begin
   DMreports.frxRSpravkaOtch.Variables['io'] := '''' +'И.О.'+ '''' ;
...
begin
  DMreports.frxRSpravkaOtch.Variables['nach'] := '''' +'Начальник управления кадров'+ '''';
    DMreports.frxRSpravkaOtch.Variables['vor'] := ''''+'О.А. Воробьева'+ '''';
...
DMreports.ADOSKCP.open;
   if   dm.ADODSFacultiesDepartment.AsInteger>1 then
 DMreports.frxRKCP1.Variables['otd'] := '''' +'Заочное отделение' + ''''
else
  DMreports.frxRKCP1.Variables['otd'] := '''' +'Очное отделение' + '''';
...
begin
      ...
      DMreports.AdodsOlandUdar.Parameters[1].Value:=combobox20.ItemIndex;
           ...
           DMreports.frxROlandUdar.Variables['ses'] := '''' +'зимней' + ''''
             end
              else
              begin
           s:=('01.06.'+inttostr(yearof(date)));
               DMreports.frxROlandUdar.Variables['ses'] := '''' +'летней' + '''';
              end;
...
begin
 if (dm.adodsStudentDataStatus.Value=10) or (dm.adodsStudentDataStatus.Value=10)  then
          ...
          ShowMessage('Недоступно для данного студента')
          else
 begin
...
//  if (dm.adodsWorkCurrDisc.RecordCount > 0)  then
   if (dm.adodsWorkCurriculumidPlan.AsInteger > 0)  then
    ...
    begin
      if(MessageBox(handle, ('Все предыдущие связи будут утеряны.Нажмите ОК, если хотите продолжить  '), 'Внимание!', MB_OKCANCEL)=IDOK)
      then
...
begin
  fDirectory.pcDirectory.HideAllTabs;
//    fDirectory.pcDirectory.Pages[1].Show;
      fDirectory.Caption := 'Учебные планы(Загрузка)';
       dm.adodoldrup.close;
if dm.ADODSFacultiesDepartment.asinteger=1 then
  begin
...
begin
  ...
  begin
   ...
   //Автозаполнение при регистрации в общежитии
procedure TfMain.RadioGroup10Click(Sender: TObject);
begin
case RadioGroup10.ItemIndex of
0: DBEditEh2.Text:='Цвиллинга 64/1' ;
1: DBEditEh2.Text:='Цвиллинга 64/2' ;
...
begin
  ...
  // заносим данные о выбранном студенте
  dm.adodsStudentData.Close;
  ...
  QuickSearch(TTVmain, dm.ADODSStudentListID.AsInteger);
    ...
    //Прорисовка таблицы со студентами
...
var
  ...
  i: Integer;
begin
  // -------убираем значок сортировки у других столбцов
  ...
  // ------------готово, теперь, если уже есть стрелка, и мы на неё жмем, то переворачиваем ее
...
begin
  ...
  begin
    ...
    GroupBox18.Visible:=false;
    panel32.Caption:='Заочное отделение'
  end
  else
  begin
    ...
    if TTVmain.Visible then
       ...
       GroupBox18.Visible:=true;
           panel32.Caption:='Очное отделение'
  end;
  ...
  // ---   какие-то нужные махинации с закладками
  ...
  // -------убираем значок сортировки у ВСЕХ столбцов в ГРИДЕ
  TDBGmain.Columns[3].Title.Caption := 'Фамилия';
  TDBGmain.Columns[4].Title.Caption := 'Имя';
  TDBGmain.Columns[5].Title.Caption := 'Отчество';
  TDBGmain.Columns[6].Title.Caption := 'Условия обучения';
...
dm.ADODSCloseProg.open;
 if (dm.ADODSCloseProgonoff.Value)=3 then
   begin
    ...
    ShowMessage('Через 3 минуты программа автоматически закроется,пожалуйста сохраните все изменения');
    Timer2.Enabled:=true;
...
//  JpegIm.Assign(bm);
//  JpegIm.CompressionQuality := 100; {от 0 до 100 вроде качество можно ставить }
//  JpegIm.Compress;
    ...
    2: // ----группа
      begin
       ...
       TabSheet6.TabVisible:=false;
        ...
        lGroupName.Caption :=
          'Название: ' + dm.adodsGroupDataGroupName.AsString;

        if (pos('магистр', ansilowercase
              (dm.adodsWorkCurriculumQualification.AsString)) > 0) or
          (pos('бакалав', ansilowercase
              (dm.adodsWorkCurriculumQualification.AsString)) > 0) then
        begin
          fMain.lNSpec.Caption := 'Код направления';
        ...
        begin
          fMain.lNSpec.Caption := 'Код специальности';
...
//      SpeedButton26.Visible:=true
  ...
  N16.Enabled := true;
 panel16.Caption:='Итого:'+inttostr(dm.ADODSStudentList.RecordCount);
end;
  ...
  case PreviousSelectedNode.Level of
    0:
      begin
        ...
        // ---карточка
        if (PreviousSelectedNode.Text <> 'Архив') then
