...
begin
 if (MessageDlg('Вы действительно хотите удалить дисциплину  "'+ dm.adodsSessionSubjectName.AsString+'"' +#13 + 'Данное действие отменить будет невозможно! Продолжить?', mtConfirmation,[mbYes, mbNo], 0) = mrYes) then
  begin
...
begin
  ...
  if(MessageBox(handle, 'Сохранить изменения?', 'Внимание!', MB_YESNO)=IDYES)
  then
  begin
...
begin
  ...
  then
      ...
      dbgSession.Columns[5].PickList.Add('Отлично');
      dbgSession.Columns[5].PickList.Add('Хорошо');
      dbgSession.Columns[5].PickList.Add('Удовлетворительно');
      dbgSession.Columns[5].PickList.Add('Неудовлетворительно');
      ...
      dbgSession.Columns[5].KeyList.Add('7');
      dbgSession.Columns[5].PickList.Add('Зачтено');
      dbgSession.Columns[5].PickList.Add('Не зачтено');
      dbgSession.Columns[5].PickList.Add('');
      end;
...
FAddErrorWCD.Label2.Caption:='Изменяемый предмет: '  + dm.adodsSessionSubjectName.AsString;
FAddErrorWCD.Label3.Caption:='Семестр: '  + dm.adodsSessionSemester.AsString;
...
begin
  if(MessageBox(handle, 'Отменить изменения?', 'Внимание!', MB_YESNO)=IDYES)
