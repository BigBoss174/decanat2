...
begin
  Showmessage('Дисциплина не имеет формы отчетности');
  exit;
end;
...
////  DM.dsWorkCurrDisc.DataSet := DM.adodsWorkCurrDisc;
 label6.caption:='Начало добавления...';
ProgressBar1.Position:=ProgressBar1.Position+10;
sleep(1000);
ProgressBar1.Position:=ProgressBar1.Position+10;
  if(WorkCurriculumModified)
  then
      if(MessageBox(handle, 'Перед добавлением новой дисциплины необходимо сохранить изменения в плане! Нажмите "ОК", чтобы сохранить изменения и добавить дисциплину, либо "Отмена", чтобы отменить действие', 'Внимание!', MB_OKCANCEL)=IDOK)
      then
          begin
      ...
      ProgressBar1.Position:=ProgressBar1.Position+10;
       ...
       label6.caption:='Перестроение списка';
      ProgressBar1.Position:=ProgressBar1.Position+10;
sleep(1000);
ProgressBar1.Position:=ProgressBar1.Position+10;
  DM.adodsWorkCurrDisc.Close;
...
//DM.dsWorkCurrDisc.DataSet := DM.adodsWorkCurrDisc;
  ...
  then
      begin
      msgResult := MessageBox(handle, 'В рабочем плане сделаны изменения! Сохранить?', 'Внимание!', mb_YesNoCancel);
...
begin
  ...
  pMessage := pChar('Удалить дисциплину "'+DM.adodsWorkCurrDiscSubjectName.AsString+'",семестр '+DM.adodsWorkCurrDiscSemester.AsString+'?');
  if(MessageBox(handle, pMessage, 'Внимание!', MB_YESNO)=IDYES)
  then
  begin
  label6.caption:='Подготовка';
  ProgressBar1.position:=0;
Panel3.visible:=true;
 ...
 dm.ADODUpdSessionOld.Parameters[1].Value:=dm.adodsWorkCurriculumidGroup.AsInteger;
dm.ADODUpdSessionOld.open;
 label6.caption:='Обновление сессии';
ProgressBar1.Position:=ProgressBar1.Position+10;
sleep(2000);
ProgressBar1.Position:=ProgressBar1.Position+30;
i:=fmain.TTVmain.Selected.GetNext;
