...
type
  TDMreports = class(TDataModule)
    ...
    frxRKCPTwo: TfrxReport;
    frxRFilial: TfrxReport;
    ...
    frxDBDPPS: TfrxDBDataset;
    frxRPPS: TfrxReport;
    ADODSrokObuchStudyingEndYear: TWideStringField;
...
if CompareText(VarName, 'ControlFormTitle') = 0
  ...
  then
    begin
       if DM.adodsWorkCurrDiscControlTest.AsBoolean then
          Value :=  'сдаче зачЄта';
       if DM.adodsWorkCurrDiscControlExamination.AsBoolean then
          Value := 'сдаче экзамена';
       if DM.adodsWorkCurrDiscControlCourseWork.AsBoolean then
          Value :='защите курсовой работы';
  ...
  then
    begin
       if DM.adodsWorkCurrDiscControlTest.AsBoolean then
          Value := '"зачтено" <u>          </u>,"не зачтено" <u>          </u>';
       if DM.adodsWorkCurrDiscControlExamination.AsBoolean then
          Value :='"отлично" <u>          </u>,"хорошо" <u>          </u>,"удовлетворительно" <u>          </u>,"неудовлетворительно" <u>          </u>';
       if DM.adodsWorkCurrDiscControlCourseWork.AsBoolean then
          Value :='"отлично" <u>          </u>,"хорошо" <u>          </u>,"удовлетворительно" <u>          </u>,"неудовлетворительно" <u>          </u>';
  end;
